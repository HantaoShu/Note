...
#### 9.2 性能度量
考虑两类 簇内相似度高 簇间相似度低    
根据聚类划分和模型原本划分可以将划分结果分成四种(2 2交叉) SS SD DS DD 根据这四个可以得到簇间指标 Jaccard系数 FM指数 Rand指数   
在得到每个分类的均值以及全集的均值后可以通过计算他们得到DB指数以及Dunn指数 这是判断簇内指标

#### 9.3 距离计算
距离需要满足一些条件  
1. 非负性
2. 同一性(若两点距离为0 则是相同两点)
3. 对称性
4. 直递性(三角不等式) 若不满足则称非度量距离

明克夫斯基距离  k=1 曼哈顿 k=2 欧式   
分类: 连续和离散  有序和无序({1 2 3} 对 {飞机 火车 轮船})  
对于无序的属性可以采用VDM 根据样本所在某处比例与某属性比例差的幂判断距离  
若混合 则有序在无序前 可以加权

#### 9.4 原型聚类
原型聚类是指假设聚类结构能通过一组原型刻画  原型是指样本空间中具有代表性的点
1. k均值算法
2. 学习向量量化(LVQ)  
假设训练数据有类别标记 监督性学习  
算法先对原型向量进行初始化 之后随机选择样本 判断距离 若是最近的原型向量 增加原型向量与该向量距离 否则减少  之后可以进行Voronoi划分 即属于最近的原型向量
3. 高斯混合聚类 利用高斯分布模型以及EM算法 E步 获得后验概率 M步 计算a u sigma

#### 9.5 密度聚类
算法假设聚类结构能通过样本分布的紧密程度确认 考虑样本间的可连接性
DBSCAN算法  
定义了以下概念:
- epsion-临域 对于每个样本 与该样本距离小于epsion的样本集合
- 核心对象 对于某个样本 他的epsion-临域至少有MinPts个样本
- 密度直达 xj在xi的epsion-邻域中 且xi是核心对象 则xj由xi密度直达(通常不满足对称性)
- 密度可达 存在一个序列 通过相邻接密度直达 那么头尾密度可达
- 密度相连 存在xk使xi和xj通过xk密度可达  

簇的定义 由密度可达关系导出最大的密度相连样本集合

#### 9.6 层次聚类
层次聚类试图在不同层次对数据集合进行划分 从而形成树形的聚类结构 可以自顶向下也可以自底向上  
AGNES算法 先将每个样本每个样本当成一个簇 然后合并直到数目要求即可 合并可以根据最小距离 最大距离 平均距离等

# Chapter 10 降纬与度量学习
#### 10.1 k临近学习(KNN)
对于测试样本 通过某种距离度量找到训练集合最靠近的k个邻居 根据邻居的信息进行预测 如投票法 平均法  
k临近是一种懒惰学习的著名代表 训练阶段仅仅把样本收集保存 训练开销为0 相应的 在训练阶段就对样本进行处理的方法叫做急切学习  
当人已样本在任意小距离都有训练样本的情况下 泛化错误不超过贝叶斯分类器两倍

#### 10.2 低纬迁入
在KNN中假设在任意样本在任意小的距离都有训练样本 而这个条件很难实现 尤其是当纬度高的时候 当维度高的情况下会有数据样本稀疏 计算困难的问题  
因此需要将高维映射到低维  
多维缩放算法 MDS
1. 通过距离矩阵D 计算dist 
2. 根据dist计算B
...
#### 10.3 主成分分析
寻找一个超平面 满足最近重构性(样本点到这个超平面的距离都足够近) 最大可分性(样本点在这个超平面上投影尽可能分开)  
从最大可分性比较号理解  若投影尽可能分开  那么应该投影后方差尽可能大  
PCA算法   
1. 对数据样本中心化
2. 计算协方差矩阵
3. 对协方差矩阵特征值分解 
...
#### 10.4 核化线性降维
在PCA算法中加入核 kernel  
之前两种方法从高维到低维的函数都是线性的 若需要非线性者需要用kernel  
具体也就是将XX<sup>T</sup> 改成核函数

#### 10.5 流形学习
流形学习数据的局部空间应该具有欧氏空间的性质 通过这个进行降维度
1. 等度量映射  
先找k临近  如果在k临近内 则计算距离为欧式距离 否则为无穷大  用最短路算法 计算所有距离 再将这距离作为MDS算法输入   
构图时也可将k临近改成epsion邻域
2. 局部线性嵌入  
认为高低纬度样本邻域存在相同的线性关系  
LLE算法
...
#### 10.6 度量学习
通过机器学习算法 学习得到一个距离度量  
将距离推广到马氏距离 之后优化M 最后对M进行特征值分解 达到降维目的
